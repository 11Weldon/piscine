...
def print_test(target: Page, expected_result: bool):
    print("================START===============")
    print(str(target))
    print("===============IS_VALID=============")
    assert target.is_valid() == expected_result
...
def test_Table():
    ...
    target = Page(Table([Tr()]))
    print_test(target, True)
    target = Page(Table([H1(Text("Hello World!"))]))
...
def test_Tr():
    ...
    target = Page(Tr())
    print_test(target, False)
    target = Page(Tr([Th(Text("title")), Th(Text("title")), Th(Text("title")),
                      Th(Text("title")), Th(Text("title"))]))
    print_test(target, True)
    target = Page(Tr([Td(Text("content")), Td(Text("content")), Td(Text("content")),
                      Td(Text("content")), Td(Text("content")), Td(Text("content"))]))
    print_test(target, True)
    target = Page(Tr([Th(Text("title")), Td(Text("content"))]))
    print_test(target, False)
...
def test_Ul_OL():
    ...
    target = Page(Ol())
    print_test(target, False)
    target = Page(Ul([Li(Text('test'))]))
    print_test(target, True)
    target = Page(Ol([Li(Text('test'))]))
    print_test(target, True)
    target = Page(Ul([Li(Text('test')), Li(Text('test'))]))
    print_test(target, True)
    target = Page(Ol([Li(Text('test')), Li(Text('test'))]))
    print_test(target, True)
    target = Page(Ul([Li(Text('test')), H1(Text('test'))]))
    print_test(target, False)
    target = Page(Ol([Li(Text('test')), H1(Text('test'))]))
    print_test(target, False)
...
def test_Span():
    ...
    target = Page(Span())
    print_test(target, True)
    target = Page(Span([Text("Hello?"), P(Text("World!"))]))
    print_test(target, True)
    target = Page(Span([H1(Text("World!"))]))
    print_test(target, False)
...
def test_P():
    ...
    target = Page(P())
    print_test(target, True)
    target = Page(P([Text("Hello?")]))
    print_test(target, True)
    target = Page(P([H1(Text("World!"))]))
    print_test(target, False)
...
def test_Title_H1_H2_Li_Th_Td():
    ...
    for c in [H1, H2, Li, Th, Td]:
        target = Page(c())
        print_test(target, False)
        target = Page(c([Text("Hello?")]))
        print_test(target, True)
        target = Page(c([H1(Text("World!"))]))
        print_test(target, False)
        target = Page(c([Text("Hello?"), Text("Hello?")]))
        print_test(target, False)
...
def test_Body_Div():
    ...
    for c in [Body, Div]:
        ...
        target = Page(c([H1(Text("World!"))]))
        print_test(target, True)
        target = Page(c([Text("Hello?"), Span()]))
        print_test(target, True)
        target = Page(c([Html(), c()]))
        print_test(target, False)
...
def test_Html():
    ...
    target = Page(Html())
    print_test(target, False)
    target = Page(Html([Head([Title(Text("Hello?"))]), Body([H1(Text("Hello?"))])]))
    print_test(target, True)
    target = Page(Html(Div()))
    print_test(target, False)
